...
def test_eval_keras():
    ...
    res = spn.eval(input_vec.T)
    print('First evaluation')
    print(res)

    spn.build_k()
    keras_lls = spn.eval_func_keras([input_vec])
    print(keras_lls)

    assert_array_almost_equal(numpy.array(res), keras_lls)
...
def test_eval_marg():
    ...
    n_nodes_layer_2 = 4
    layer_2_prod_nodes = [ProductNode(var_scope={0, 1, 2}) for i in range(n_nodes_layer_2)]
    layer_2_prod_nodes[0].add_child(layer_1_sum_nodes[0])
    layer_2_prod_nodes[0].add_child(layer_1_sum_nodes[2])
    layer_2_prod_nodes[0].add_child(layer_1_sum_nodes[4])
    layer_2_prod_nodes[1].add_child(layer_1_sum_nodes[1])
    ...
    all_worlds = sample_all_worlds(3)
    print(all_worlds)
    res = spn.eval(all_worlds)
    print(res)
    alls = scipy.misc.logsumexp(res)
    print(alls)

    #
    # marg eval
    mask = {layer_2_prod_nodes[1]}
    mres = spn.eval_marg(all_worlds, mask)
    print(mres)
    malls = scipy.misc.logsumexp(mres)
    print(malls)
