...
class Layer(object):
    ...
    def eval_marg(self, marg_node_set):
        """
        layer bottom-up evaluation
        """
        for node in self._nodes:
            if node not in marg_node_set:
                node.eval()
            else:
                const_output = 1. / (2 ** len(node.var_scope))
                # print(const_output)
                node.log_val = numpy.log(const_output)
        # eval_numba(self._nodes)
    ...
    def build_k(self):
        """
        Build keras graph by dispatching its creation to each node in the layer
        """
        for node in self._nodes:
            node.build_k()
...
class CategoricalInputLayer(Layer):
    ...
    def build_k(self, input):
        """
        Build keras graph by dispatching its creation to each node in the layer
        For input layers we have to pass the (keras placeholder) input var as well
        """
        for node in self._nodes:
            node.build_k(input)
